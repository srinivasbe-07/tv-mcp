...
function broadcast(clients, event, data) {
  ...
  clients.splice(0, clients.length, ...clients.filter(r => {
    try { r.write(msg); return true; } catch (_) { return false; }
  }));
}

function pushPattern(line) {
  const t = line.trim(); if (!t) return;
  patternLog.push(t); if (patternLog.length > 200) patternLog.shift();
  broadcast(patternClients, 'log', { line: t });
}

function pushST(line) {
  const t = line.trim(); if (!t) return;
  stLog.push(t); if (stLog.length > 200) stLog.shift();
...
app.get('/api/config', (_req, res) => {
  try { res.json(JSON.parse(fs.readFileSync(PATTERN_CONFIG, 'utf8'))); }
...
app.get('/api/st/config', (_req, res) => {
  try { res.json(JSON.parse(fs.readFileSync(ST_CONFIG, 'utf8'))); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/st/position', (_req, res) => {
  try { res.json(JSON.parse(fs.readFileSync(POSITION_FILE, 'utf8'))); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/st/position', (req, res) => {
  try {
    ...
    res.json({ ok: true });
    pushST(`[UI] Position updated — CE:${updated.CE?.toUpperCase()}  PE:${updated.PE?.toUpperCase()}`);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/events', (req, res) => {
  ...
  res.flushHeaders();
  patternLog.slice(-50).forEach(line =>
    res.write(`event: log\ndata: ${JSON.stringify({ line })}\n\n`)
  );
  res.write(`event: status\ndata: ${JSON.stringify(getStatus())}\n\n`);
  patternClients.push(res);
  req.on('close', () => { patternClients = patternClients.filter(c => c !== res); });
});
...
app.get('/api/st/events', (req, res) => {
  ...
  res.setHeader('Connection', 'keep-alive');
  res.flushHeaders();
  stLog.slice(-50).forEach(line =>
    res.write(`event: log\ndata: ${JSON.stringify({ line })}\n\n`)
  );
  res.write(`event: status\ndata: ${JSON.stringify(getStatus())}\n\n`);
  stClients.push(res);
  req.on('close', () => { stClients = stClients.filter(c => c !== res); });
});
...
app.post('/api/tv/start', (_req, res) => {
  ...
  tvProc = spawn('powershell', ['-ExecutionPolicy', 'Bypass', '-File', LAUNCH_TV_PS], { cwd: ROOT });
  tvProc.stdout.on('data', d => {
    const line = d.toString().trim();
    if (!line) return;
    pushPattern(`[TV] ${line}`); pushST(`[TV] ${line}`);
    if (line.includes('CDP is ready') || line.includes('already running')) {
      tvReady = true; broadcastStatus();
    }
  });
  tvProc.stderr.on('data', d => { pushPattern(`[TV] ${d}`); pushST(`[TV] ${d}`); });
  tvProc.on('close', code => {
    tvProc = null;
    if (code !== 0) tvReady = false;
    const msg = `[TV] Script exited (${code})`;
    pushPattern(msg); pushST(msg);
    broadcastStatus();
  });
  broadcastStatus();
  res.json({ ok: true });
...
app.post('/api/tv/stop', (_req, res) => {
  exec('taskkill /IM TradingView.exe /F', () => {
    tvReady = false; tvProc = null;
    pushPattern('[UI] TradingView stopped'); pushST('[UI] TradingView stopped');
    broadcastStatus();
  });
  res.json({ ok: true });
});
...
app.post('/api/monitor/start', (_req, res) => {
  ...
  monitorProc.stderr.on('data', d => d.toString().split('\n').forEach(l => { if (l.trim()) pushPattern(`[ERR] ${l.trim()}`); }));
  monitorProc.on('close', code => {
    pushPattern(`[UI] Pattern monitor stopped (${code})`);
    monitorProc = null; broadcastStatus();
  });
  broadcastStatus();
  res.json({ ok: true });
});
...
app.post('/api/monitor/restart', (_req, res) => {
  ...
  const doStart = () => {
    ...
    monitorProc.stdout.on('data', d => d.toString().split('\n').forEach(pushPattern));
    monitorProc.stderr.on('data', d => d.toString().split('\n').forEach(l => { if (l.trim()) pushPattern(`[ERR] ${l.trim()}`); }));
    monitorProc.on('close', code => { pushPattern(`[UI] Pattern monitor stopped (${code})`); monitorProc = null; broadcastStatus(); });
    broadcastStatus();
  };
  if (monitorProc) {
...
app.post('/api/st/start', (_req, res) => {
  ...
  stProc.stderr.on('data', d => d.toString().split('\n').forEach(l => { if (l.trim()) pushST(`[ERR] ${l.trim()}`); }));
  stProc.on('close', code => {
    pushST(`[UI] Supertrend monitor stopped (${code})`);
    stProc = null; broadcastStatus();
  });
  broadcastStatus();
  res.json({ ok: true });
});
...
app.post('/api/st/restart', (_req, res) => {
  ...
  const doStart = () => {
    ...
    stProc.stdout.on('data', d => d.toString().split('\n').forEach(pushST));
    stProc.stderr.on('data', d => d.toString().split('\n').forEach(l => { if (l.trim()) pushST(`[ERR] ${l.trim()}`); }));
    stProc.on('close', code => { pushST(`[UI] Supertrend monitor stopped (${code})`); stProc = null; broadcastStatus(); });
    broadcastStatus();
  };
  if (stProc) {
