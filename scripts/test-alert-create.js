...
try {
  ...
  for (const a of alerts) {
    ...
    if (d.success) {
      console.log(`✓`);
      console.log(
        `  price verified : ${d.priceVerified ? '✓' : '✗  WARN: price may not have been set correctly'}`
      );
      if (a.once)
        console.log(
          `  once btn       : ${d.onceBtnFound ? '✓' : `✗ NOT FOUND — options seen: [${(d.onceOptions || []).join(' | ')}]`}`
        );
      console.log(`  price selector : ${d.priceInputSrc}`);
      console.log(
        `  name set       : ${d.nameSet ? '✓' : '✗  WARN: name not visible in panel after creation'}`
      );
      console.log(`  name method    : ${d.nameSetMethod}`);
      if (!d.nameSet && d.nameDiag) {
        const nd = d.nameDiag;
        console.log(
          `  name btn found : ${nd.strategyB_btnFound} (${nd.strategyB_btnText || 'n/a'})`
        );
        console.log(
          `  name input     : found=${nd.nameInputFound} tag=${nd.nameInputTag || 'n/a'} cls=${nd.nameInputCls || 'n/a'}`
        );
        if (nd.subDialogInputs) console.log(`  sub inputs     : ${nd.subDialogInputs.join(' | ')}`);
        if (!nd.strategyB_btnFound) {
          console.log(
            `  visible btns   : ${(nd.visibleBtns || [])
  ...
  let listData = {};
  try {
    listData = JSON.parse(listR?.content?.[0]?.text || '{}');
  } catch (_) {
    /* ignore */
  }
  const found = (listData.alerts || []).filter((a) =>
    ['TradeEntry', 'TradeSL', 'TradeTarget'].includes(a.name)
  );
  ...
  const missing = ['TradeEntry', 'TradeSL', 'TradeTarget'].filter(
    (n) => !found.some((a) => a.name === n)
  );
  ...
  console.log(
    '\n' +
      (allOk && found.length === 3
        ? '✓ All 3 alerts created and visible'
        : '⚠ Some alerts failed — check output above')
  );
} catch (e) {
