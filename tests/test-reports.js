...
test('accepts bar within ±90s tolerance', () => {
  ...
  return findPrice(bars, alertUnix) === 99.00;
});
...
test('picks closest bar when multiple candidates', () => {
  ...
  return findPrice(bars, alertUnix) === 20.00;
});
...
test('reach >= 20 + small loss → price reach upto X', () => autoNotes('NIFTY', 200, 196, 42) === 'price reach upto 42 points');
test('reach >= 20 + profit → price reach upto X',     () => autoNotes('NIFTY', 200, 215, 25) === 'price reach upto 25 points');
test('reach = exactly 20 → price reach upto X',       () => autoNotes('NIFTY', 200, 198, 20) === 'price reach upto 20 points');
test('reach < 20 + small loss → PINE SCRIPT SL',      () => autoNotes('NIFTY', 200, 190, 10) === 'PINE SCRIPT SL');
...
test('SENSEX reach >= 20 → price reach note',          () => autoNotes('SENSEX', 500, 495, 22) === 'price reach upto 22 points');
...
function calcMaxReach(entryPrice, tradeBars) {
  ...
  return parseFloat(Math.max(0, ...tradeBars.map(b => parseFloat(b.high) - entryPrice)).toFixed(2));
}
...
test('reach rounded to 2 decimal places', () =>
  calcMaxReach(200, [{ high: 200.333 }]) === 0.33);
...
test('before from → false',             () => tradeInAnyRange('09:00', [{ from: '09:15', to: '11:00' }]) === false);
test('after to → false',                () => tradeInAnyRange('12:00', [{ from: '09:15', to: '11:00' }]) === false);
test('on from boundary → true',         () => tradeInAnyRange('09:15', [{ from: '09:15', to: '11:00' }]) === true);
test('on to boundary → true',           () => tradeInAnyRange('11:00', [{ from: '09:15', to: '11:00' }]) === true);
test('matches second of two ranges',    () =>
  tradeInAnyRange('14:30', [{ from: '09:15', to: '11:00' }, { from: '14:00', to: '15:30' }]) === true);
test('matches neither range → false',   () =>
  tradeInAnyRange('12:00', [{ from: '09:15', to: '11:00' }, { from: '14:00', to: '15:30' }]) === false);
test('missing entryTime → true',        () => tradeInAnyRange('', [{ from: '09:15', to: '11:00' }]) === true);
test('only from set, time >= from → true',  () => tradeInAnyRange('10:00', [{ from: '09:15', to: '' }]) === true);
test('only from set, time < from → false',  () => tradeInAnyRange('09:00', [{ from: '09:15', to: '' }]) === false);
test('only to set, time <= to → true',  () => tradeInAnyRange('09:30', [{ from: '', to: '10:00' }]) === true);
...
test('numeric maxReach field → returned directly', () => getEffectiveMaxReach({ maxReach: 42 }) === 42);
...
test('decimal reach value parsed',                 () =>
  getEffectiveMaxReach({ notes: 'reach 31.5' }) === 31.5);
test('no reach in notes → 0',                      () =>
  getEffectiveMaxReach({ notes: 'SL HIT' }) === 0);
...
test('"PINE SCRIPT SL HIT" → pine-sl (PINE first)', () => notesClass('PINE SCRIPT SL HIT') === 'pine-sl');
test('empty string → empty',                         () => notesClass('') === '');
test('null → empty',                               () => notesClass(null) === '');
test('unrecognized text → empty',                  () => notesClass('took profit') === '');
test('"price reach upto 42 points" → empty (not SL type)', () => notesClass('price reach upto 42 points') === '');
...
function tradePassesFilters_test(t, ranges, noteFilter, minReach) {
  ...
  if (anyNoteChecked) {
    ...
    const pass = (noteFilter.slHit  && n.includes('SL HIT') && !n.includes('PINE SCRIPT')) ||
...
test('in range → passes', () =>
  tradePassesFilters_test({ entryTime: '10:00', notes: '' }, [{ from: '09:15', to: '11:00' }], NO_FILTER, null) === true);
test('outside range → blocked', () =>
  tradePassesFilters_test({ entryTime: '08:00', notes: '' }, [{ from: '09:15', to: '11:00' }], NO_FILTER, null) === false);

section('tradePassesFilters — notes filter');
test('slHit=true, trade has SL HIT → passes', () =>
  tradePassesFilters_test({ entryTime: '10:00', notes: 'SL HIT' }, [], { slHit: true, pineSL: false }, null) === true);
test('slHit=true, trade has PINE SCRIPT SL HIT → blocked (pine takes precedence)', () =>
  tradePassesFilters_test({ entryTime: '10:00', notes: 'PINE SCRIPT SL HIT' }, [], { slHit: true, pineSL: false }, null) === false);
test('pineSL=true, trade has Pine Script SL Hit → passes', () =>
  tradePassesFilters_test({ entryTime: '10:00', notes: 'Pine Script SL Hit' }, [], { slHit: false, pineSL: true }, null) === true);
test('pineSL=true, trade has plain SL HIT → blocked', () =>
  tradePassesFilters_test({ entryTime: '10:00', notes: 'SL HIT' }, [], { slHit: false, pineSL: true }, null) === false);
test('both false, no notes → passes', () =>
  tradePassesFilters_test({ entryTime: '10:00', notes: '' }, [], NO_FILTER, null) === true);
test('both true, trade profit (no notes) → blocked', () =>
  tradePassesFilters_test({ entryTime: '10:00', notes: '' }, [], { slHit: true, pineSL: true }, null) === false);
test('slHit=true, trade has "price reach upto X" → blocked (not SL HIT)', () =>
  tradePassesFilters_test({ entryTime: '10:00', notes: 'price reach upto 42 points' }, [], { slHit: true, pineSL: false }, null) === false);
test('pineSL=true, trade has "price reach upto X" → blocked (not PINE SCRIPT)', () =>
  tradePassesFilters_test({ entryTime: '10:00', notes: 'price reach upto 42 points' }, [], { slHit: false, pineSL: true }, null) === false);
test('both false + "price reach upto X" notes → passes (no filter active)', () =>
  tradePassesFilters_test({ entryTime: '10:00', notes: 'price reach upto 42 points' }, [], NO_FILTER, null) === true);

section('tradePassesFilters — reach filter');
test('reach >= min → passes',        () =>
  tradePassesFilters_test({ entryTime: '10:00', notes: '', maxReach: 25 }, [], NO_FILTER, 20) === true);
test('reach < min → blocked',        () =>
  tradePassesFilters_test({ entryTime: '10:00', notes: '', maxReach: 15 }, [], NO_FILTER, 20) === false);
test('reach = min exactly → passes', () =>
  tradePassesFilters_test({ entryTime: '10:00', notes: '', maxReach: 20 }, [], NO_FILTER, 20) === true);
test('minReach=0 → not filtered',    () =>
  tradePassesFilters_test({ entryTime: '10:00', notes: '', maxReach: 0  }, [], NO_FILTER, 0)  === true);
test('reach in old notes text → parsed and compared', () =>
  tradePassesFilters_test(
    { entryTime: '10:00', notes: 'price reach upto 30 points' }, [], NO_FILTER, 25
  ) === true);
...
test('tgtPts = exitNSL - entry for small profit', () => recalc('NIFTY', 200, 210, 10).tgtPts === 10);
...
test('loss 10pts → pNSL = -6500', () =>
  recalc('NIFTY', 200, 190, 10).pNSL === -6500);
...
test('unsorted trades sorted by entryTime', () => {
  ...
  return sorted[0].entryTime === '09:15' &&
...
test('same entryTime → stable (order preserved)', () => {
  const trades = [{ entryTime: '10:00', id: 1 }, { entryTime: '10:00', id: 2 }];
  const sorted = sortTrades(trades);
