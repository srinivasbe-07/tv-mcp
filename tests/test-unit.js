...
assert(
  'CE uses C not CE',
  buildSymbol(23950, 'CE', tuesday).includes('C23950') &&
    !buildSymbol(23950, 'CE', tuesday).includes('CE'),
  buildSymbol(23950, 'CE', tuesday)
);
assert(
  'PE uses P not PE',
  buildSymbol(24150, 'PE', tuesday).includes('P24150') &&
    !buildSymbol(24150, 'PE', tuesday).includes('PE'),
...
assert(
  'Monday   → next Tuesday (May 26)',
  fromMon.getUTCDate() === 26,
  `got ${fromMon.toUTCString()}`
);
assert('Tuesday  → same day (May 26)', fromTue.getUTCDate() === 26, `got ${fromTue.toUTCString()}`);
assert(
  'Wednesday→ next Tuesday (Jun 2)',
  fromWed.getUTCDate() === 2,
  `got ${fromWed.toUTCString()}`
);
assert(
  'Sunday   → next Tuesday (May 26)',
  fromSun.getUTCDate() === 26,
  `got ${fromSun.toUTCString()}`
);
...
const tueTuesdayHolidays = [
  ...
  { mon: [2026, 10, 9], tue: [2026, 10, 10], label: 'Nov 10 Diwali → Mon Nov 9', expectDate: 9 },
  {
    mon: [2026, 10, 23],
    tue: [2026, 10, 24],
    label: 'Nov 24 Gurpurb → Mon Nov 23',
    expectDate: 23,
  },
];
